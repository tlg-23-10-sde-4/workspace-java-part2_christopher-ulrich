...
public class EmployeeStreamsTest {
    ...
    public void testSalaryLessThanSortNaturalOrder() {
        List<Employee> employees = allEmployees.stream()
                .filter(emp -> emp.getSalary() < 40_000.0)
...
//                .sorted( (emp1, emp2) -> emp1.getSalary().compareTo(emp2.getSalary()))
                .sorted(comparing(Employee::getSalary))   // emp -> emp.getSalary()
                .collect(toList());
        dump(result);
    }
...
//                .sorted( (emp1, emp2) -> emp1.getName().compareTo(emp2.getName()))
                .sorted(comparing (Employee::getName))    // emp -> emp.getName()
                .collect(toList());
    ...
    public void findAllNames_hired2010Later() {
        List<String> names = allEmployees.stream()
                .filter(emp -> emp.getHireDate().getYear() > 2009 )
                .map(emp -> emp.getName())  // Stream<String>, can be changed to a Method Reference
                .sorted()
                .collect(toList());
    ...
    public void payAll_salaryAtMost50000_lowestPaidFirst() {
        allEmployees.stream()
                .filter(emp -> emp.getSalary() <= 50_000.0)
                .sorted( (emp1, emp2) -> emp1.getSalary().compareTo(emp2.getSalary()))
                .forEach(Employee::pay);    // lambda is emp -> emp.pay()
    ...
    public void giveRaise_salaryLessThan50000_hiredAfter2000_lowestPaidFirst() {
        List<Employee> raises = allEmployees.stream()
//                .filter(emp -> emp.getSalary() < 50_000.0)
//                .filter(emp -> emp.getHireDate().getYear() > 2000)
    ...
    public void anyEmployees_nameStartWithZ() {
        boolean lastNameStartsWithZ = allEmployees.stream()
                .anyMatch(emp -> emp.getName().startsWith("Z"));
        System.out.println(lastNameStartsWithZ);
    }
    ...
    public void allEmployeesHired2000_orLater() {
        boolean hiredBefore2000 = allEmployees.stream()
                .anyMatch(emp -> emp.getHireDate().getYear() < 2000);
        System.out.println(hiredBefore2000);
    }
    ...
    public void groupingByYearOfHire() {
        ...
        Map<Integer, List<Employee>> treeMap = new TreeMap<>(map);
        dump(treeMap);
    }
    ...
    public void groupingByYearOfHireCount() {
        Map<Integer, Long> map = allEmployees.stream()
    ...
    public void twoPartComparator() {
        ...
        animals.sort(Comparator.comparing(String::length).thenComparing(naturalOrder()));    // animals -> animals.length()
